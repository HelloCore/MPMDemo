	...
	if (options.droppable) {
		...
		$(document)
			.bind('dragstart', function(ev, ui) {
				var _e = ev.target;
				var e = $(_e);
				if (!e.parents('.fc').length || e.hasClass('icon-copy-timesheet')) { // not already inside a calendar
...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		if(holiday && holiday[date.format()]){
		...
		if (showNumbers) {
			html += "<div class='fc-day-number'>" + 
						" <span class='label label-effort pull-right' data-date='"+date.format()+"'>8</span>"+
						"<span class='label-day pull-right'>"+date.date()+"</span>"+
						"<br class='clearfix' />"+
					"</div>";
		}
	...
	function dayDBClick(ev){
		var date = calendar.moment($(this).data('date'));
		trigger('dayDBClick', this, date, ev);
	}
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg) {
		...
		var classes = ['fc-event', 'fc-event-vert', 'panel-event'];
		...
		html +=
			" class='" + classes.join(' ') + " panel panel-"+event.eventColor+"'"+
			" style=" +
				"'" +
				"position:absolute;" +
				"top:" + seg.top + "px;" +
...
//		if(event.memo || event.editable || (seg.isEnd && isEventResizable(event))){
//			
//			html += "<div class='panel-footer' style='position:absolute;bottom:15px;width:100%;'>";
//
//			if(event.memo){
//				html+= ' <button class="btn btn-success btn-edit pull-left btn-popover" data-container="body" data-placement="auto" data-toggle="popover" data-trigger="hover" data-content="'+htmlEscape(event.memo)+'"><span class="glyphicon glyphicon-file"></span></button> ';
//			}
//			if(event.editable){
//				html+= ' <button class="btn btn-danger btn-edit pull-right "><span class="glyphicon glyphicon-trash"></span></button> '
//					+  ' <button class="btn btn-warning btn-edit pull-right"><span class="glyphicon glyphicon-pencil"></span></button> ';
//			}
//			
...
//				html +=
//					"<div class='ui-resizable-handle ui-resizable-s'><span class='glyphicon glyphicon-resize-vertical'></span></div>";
//			}
//		}
		html +=
			"</" + (url ? "a" : "div") + ">";
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		...
		var classNames = [ 'fc-event', 'fc-event-hori', 'panel-event' ];
		if (isEventDraggable(event)) {
		...
		html +=
			" class='" + classNames.join(' ') + " panel panel-"+event.eventColor+"'"+
			(event.index >= 0 ? ' data-index="'+event.index+'" ':'')+
			" style=" +
				"'" +
				"position:absolute;" +
				...
				"'" +
			">";
			// +"<div class='panel panel-warning panel-event'>";
  ...
  // </div>
		...
		if(event.temp || event.canSaveTemplate){
			html += '<span class="glyphicon glyphicon-cloud-download icon-save-template has-tool-tip" data-index="'+event.index+'" data-temp="'+event.temp+'"  data-container="body" data-placement="auto" data-toggle="tooltip" data-trigger="hover" title="Save as a Template"></span>';
			html += '<span class="glyphicon glyphicon-export icon-copy-timesheet has-tool-tip" data-drag-type="copy" data-index="'+event.index+'" data-temp="'+event.temp+'" data-container="body" data-placement="auto" data-toggle="tooltip" data-trigger="hover" title="Drag to Duplicate"></span>';
		}
		html +="<span class='label label-"+event.eventColor+" label-event-time pull-right label-event-time-tooltip' ";
		if(typeof(application.currentEffort.calEffort) == 'function' && moment.isMoment(event.start) && moment.isMoment(event.end)){
			html +='data-container="body" data-placement="auto" data-toggle="tooltip" data-trigger="hover" title="'+application.currentEffort.calEffort(event.start,event.end)+'" ';
 	 	}						
		html += " >"+event.start.format('HH:mm')+'-'+event.end.format('HH:mm')+
			"</span><br class='clearfix'/><p class='normal-text' >"+
			 htmlEscape(event.title || '') +
		 	"</p></div>"+
			"<div class='panel-body'>"+
				htmlEscape(event.desc || '') +
		...
		if(event.memo || event.timesheetID || event.temp){
			
			html += "<div class='panel-footer'>";

			if(event.memo && event.memo.length > 0){
				html+= ' <button class="btn btn-success btn-edit pull-left button-memo btn-popover" data-container="body" data-index="'+event.index+'" data-temp="'+event.temp+'"  data-placement="auto" data-toggle="popover" data-trigger="hover" data-content="'+htmlEscape(event.memo)+'"><span class="glyphicon glyphicon-comment"></span></button> ';
			}else if(event.temp){
//				html+= '<button class="btn btn-success btn-edit pull-left button-memo" data-index="'+event.index+'"  data-temp="'+event.temp+'" ><span class="glyphicon glyphicon-plus"></span> <span class="glyphicon glyphicon-comment"></span></button>';
				// html+= '<button class="btn btn-success btn-edit pull-left button-memo" data-index="'+event.index+'"  data-temp="'+event.temp+'" ><span class="glyphicon glyphicon-plus"></span></button>';
			}
			if(event.timesheetID || event.temp){
				html+= ' <button class="btn btn-danger btn-edit pull-right button-delete" data-index="'+event.index+'" data-temp="'+event.temp+'" ><span class="glyphicon glyphicon-trash"></span></button> ';
			}
			if(event.temp){
				html+= ' <button class="btn btn-warning btn-edit pull-right button-edit" data-index="'+event.index+'"  data-temp="'+event.temp+'" ><span class="glyphicon glyphicon-pencil"></span></button> ';
			}
